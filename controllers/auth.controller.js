...
export const register = async (req, res) => {
  ...
  try {
    if (!southAfricanId || !email || !password) {
      return res
        .status(400)
        .json({
          message: "South African ID, email, and password are required!",
        });
    }
    ...
    if (existingUserBySAID) {
      return res
        .status(400)
        .json({ message: "A user with this South African ID already exists!" });
    }
    ...
    const newUser = await prisma.user.create({
      data: {
        ...
        verificationToken,
        verified: false,
        role: $Enums.Role.PUBLIC_USER
      },
    });

    const verificationLink = `http://localhost:8800/api/auth/verify-email?token=${verificationToken}`;

    // This will be replace by sending email
    console.log(`Verify your email here : ${verificationLink}`);
